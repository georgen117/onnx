        ...
        for n in range(N):
            ...
            while current_bit_pos < total_zp_bits and len(unpacked_row_buf) < n_blocks_per_col:
                ...
                if (bits_available >= bits):
                    # all sub-bits are in current byte
                    value = (zero_points[byte_pos] >> bit_offset) & mask
                else:
                    #sub-bits are split across two bytes
    ...
    for n in range(N):
        ...
        for n_bpc in range(n_blocks_per_col):
            ...
            while len(unpacked_col_buf) < block_size and (len(unpacked_row_buf) + len(unpacked_col_buf)) < K and current_bit_pos < total_bits:
                ...
                if (bits_available >= bits):
                    # all sub-bits are in current byte
                    value = (X[n][byte_pos] >> bit_offset) & mask
                else:
                    # sub-bits are split accoss two bytes
                    lower_bits = (X[n][byte_pos] >> bit_offset) & mask
                    upper_bits = (X[n][byte_pos + 1] << bits_available) & mask
                    value = upper_bits | lower_bits
...
def matmulnbits_reference_implementation(
    ...
    **kwargs
    ...
    # numpy dose not support bfloat16 using float32 for the reference implementation
    if( accuracy_level == 3):
        matmul_type = np.float32
